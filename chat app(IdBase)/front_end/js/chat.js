$(document).ready(function() {
    ...
    socket.on('showNewMsg', (message) => {
        showNewMessage(message);// show new message for sender and reciever
    });

    socket.on('disconnectOneUser', (username) => {
        ...
        $(`button[value|=${username}]`).remove();//remove this user from list
    });
    ...
    const  displayMsgForReciever = (message) => {
        $('.chat-box').append(
            ...
            `<div class="media-body w-50 ml-auto mb-4">
                <p class="small text-muted text-right mb-0">${message.sender.username}</p>
    ...
    // display new message for sender and reciever
    const showNewMessage = (message) =>{
        if (message.sender.username === currentUser)
             ...
             */
            if (anotherUserInChat === message.sender.username)
                displayMsgForReciever(message);
            else {
                const unreadMark = $(`button[value|=${message.sender.username}]`).find('#unreadMark');
